...
async function main() {
  ...
  if (!user) {
    ...
    // Optional: reset password for this account (only when env is set)
    // Reason: if the user registered earlier, their password is NOT "Jagmeet@123" —
    // the seed script only sets that on *new* users.
    if (process.env.SEED_RESET_PASSWORD === 'true') {
      user.password = process.env.SEED_NEW_PASSWORD || 'Jagmeet@123';
      console.log('Password reset requested (SEED_RESET_PASSWORD=true)');
    }
    await user.save();
