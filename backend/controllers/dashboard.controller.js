...
const { buildHabitScoreSummary } = require('../utils/habitScore');
const { buildHealthDataset } = require('../utils/healthDataset');
...
async function getDashboardData(req, res, next) {
  try {
    ...
    const habitScore = buildHabitScoreSummary(habitByDay, today);
    ...
    const habitsPayload = {
      ...
      averages: {
        ...
        avgExerciseMinutesLast7Days:
          avgExerciseLast7 === null ? null : Number(avgExerciseLast7.toFixed(1)),
        ...
        avg_exercise_minutes_last_7_days:
          avgExerciseLast7 === null ? null : Number(avgExerciseLast7.toFixed(1)),
      },
      completionRate,
    ...
    const healthDataset = buildHealthDataset({
      averages: {
        sleep: avgSleepLast7,
        water: avgWaterLast7,
    ...
    const legacyInsights = generateInsights({
      averages: {
        sleep: avgSleepLast7,
        water: avgWaterLast7,
        exercise: avgExerciseLast7,
    ...
    const insights = [...new Set([...healthDataset.insights, ...legacyInsights])];
